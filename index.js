...
function getAverageGoals(data) {
    const avgArr = [];
    let avgHome = data.reduce((total, state) => total + state["Home Team Goals"],0) / data.length;
    let avgAway = data.reduce((total, state) => total + state["Away Team Goals"],0) / data.length;
    avgArr.push(avgHome);
    avgArr.push(avgAway);
    return avgArr;
